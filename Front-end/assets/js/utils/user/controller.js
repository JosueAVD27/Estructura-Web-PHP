...
function cargarDatosEnTabla(pagina, filtro) {
    $.post("../controllers/Utils/user.php?st=listar", { pagina: pagina, filtro: filtro }, function (data) {
...
function cargarDatosInactivosEnTabla(pagina, filtro) {
    $.post("../controllers/Utils/user.php?st=listar_inactivos", { pagina: pagina, filtro: filtro }, function (data) {
        datosTabla(pagina, filtro, data);
    });
}
...
function eliminarActivo(id_usuario) {
    ...
    var url_ajax = "../controllers/Utils/user.php?st=";
    var cargarDatos = function () { cargarDatosEnTabla(paginaActual, '') };

    if (id_usuario == 1 || id_usuario == 2 || id_usuario == G_id_usuario) {
...
function editar(id_usuario) {
    ...
    $.post("../controllers/Utils/user.php?st=mostrar_informacion_usuario", { id_usuario: id_usuario }, function (data) {
        ...
        $('#apellidoPaterno_usuario').val(data.apellidoPaterno_usuario);
        $('#apellidoMaterno_usuario').val(data.apellidoMaterno_usuario);
        ...
        if (id_usuario == 1 || id_usuario == 2 && G_id_usuario != 2) {
            ...
            // Oculta el campo de contraseña al hacer clic en el botón
            document.getElementById('campoCambiarGenero').style.display = 'grid';

            // Muestra el campo de cambiar contraseña al hacer clic en el botón
            document.getElementById('campoCambiarContraseña').style.display = 'grid';
...
function enviarFormulario() {
    ...
    formData.append('id_usuario_accion', G_id_usuario);
    formData.append('id_rol_accion', G_id_rol);
...
$(document).on("click", "#btnAbrirModal_usuario", function () {
    ...
    // Muestra el campo de genero al hacer clic en el botón
    document.getElementById('campoCambiarGenero').style.display = 'none';

    // Oculta el campo de cambiar contraseña al hacer clic en el botón
    document.getElementById('campoCambiarContraseña').style.display = 'none';
});
...
$(document).on("click", "#btnCerrarModal_usuario", function () {
    ...
    // Oculta el campo de genero al hacer clic en el botón
    document.getElementById('campoCambiarGenero').style.display = 'none';
