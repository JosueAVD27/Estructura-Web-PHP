...
function obtenerPaises() {
    ...
    var urlAjax = relPath + 'controllers/Utils/user.php?st=obtener_paises';
...
function obtenerRoles() {
    var relPath = definirNivel();
    // Designar la URL Ajax dependiendo del directorio actual
    var urlAjax = relPath + 'controllers/Utils/user.php?st=obtener_roles';
...
function validacionPaisCampos() {
    ...
    $('#id_pais').change(function () {
        ...
        if (urlActual.includes(ruta_registro_usuario) || urlActual.includes(ruta_perfil) || urlActual.includes(ruta_login)) {
            // Verifica si se ha seleccionado una opción válida (no vacía)
            if (selectedOption !== '') {
                // Obtén el valor del prefijoCodigoPais de la opción seleccionada
                prefijoInicial = $('option:selected', this).attr('data-prefijo-codigo-pais');
    ...
    if (urlActual.includes(ruta_registro_usuario) || urlActual.includes(ruta_perfil)) {
...
function validarNumeroCelular(idPais, telefono) {
    ...
    if (telefono.startsWith(prefijoCodigoPais)) {
        ...
        if (telefono.length === prefijoCodigoPais.length) {
            ...
            var message = 'Ingrese un número de teléfono para el prefijo (' + prefijoCodigoPais + ')';
            ...
            if (telefono.charAt(4) === "9") {
                ...
                if (formatoValido.test(telefono)) {
                    ...
                    var message = 'El número de teléfono móvil ecuatoriano debe tener el formato correcto, incluyendo el prefijo ' + prefijoCodigoPais;
                    alert_validar(title, message);
                    return false; // Retorna falso para indicar que la validación no ha pasado
                ...
                var message = 'El primer dígito después del prefijo debe ser "9" para un número de teléfono móvil Ecuatoriano';
                alert_validar(title, message);
                return false; // Retorna falso para indicar que la validación no ha pasado
            ...
            if (/^\+1\d{10}$/.test(telefono)) {
                ...
                var message = 'El número de teléfono de USA debe tener el formato correcto.';
                alert_validar(title, message);
                return false; // Retorna falso para indicar que la validación no ha pasado
...
/**
 * VALIDAR SI EL USUARIO TIENE ASIGNADA UNA ORGANIZACION Y EVITAR VOLVER A REGISTRARSE EN LA ORGANIZACION POR DEFECTO
 * @param {*} cedula 
 * @param {*} callback 
 */
// Función para validar si ya cuenta con organizaciones 
function validarOrganizacionPorDefecto(cedula, callback) {
    ...
    // Ruta del directorio de registro usuario
    if (urlActual.includes(ruta_registro_usuario)) {
        var urlAjax = relPath + "controllers/Utils/user.php?st=verificar_organizacion_por_defecto";
    }
